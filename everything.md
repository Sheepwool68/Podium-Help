...
of the code. See the section on where-chip-codes-come-from.
...
See the section on recover-missed-reads.
...
See the section on wave-starts.

**Category** — any class an entry is scored in: an age band, a grade, a division. Results within a
category are split by gender. See the section on categories.
...
are kept off the public results. See the section on publish-results.
...
see the section on command-a-box and the section on limits.
...
with a wrong one. See the section on where-chip-codes-come-from.
...
looks like on the tag, is the section on where-chip-codes-come-from.
...
in the section on entrants-and-numbers.
...
chip is one of the two ways onto it. See the section on entrants-and-numbers.
...
hold no link to it, so they cannot. See the section on limits.
...
**Almost everything that ruins a race day was decided before it.** This is the list, in the order
the mistakes actually happen.

## Weeks out: the event itself

**Check the time zone on the event.** Every time in Podium is the local wall clock the readers are
on, never UTC, and an event judged in the wrong zone is wrong by hours with nothing on the page to
show it. It is the cheapest thing to get right now and among the most annoying later.

**Pick the age basis** — age on race day, or age at 31 December. It belongs to the event and it
decides how every age category is worked out.

**Build in order: event, races, waves, categories, timing points, entrants.** Each needs the one
above it to exist, and the commonest import failure is a list arriving before the races and
categories it refers to. See the section on set-up-an-event.

**Leave the race window generous.** It is the span the race can be timed in, not the gun, and a
finisher arriving after it closes is discarded as though the box never reported.

## The week before: entries and chips

- **Import the list and look at the column mapping** before anything is written. Nothing commits
  until you say so, and an import can be undone.
- **Get the chips against people.** A chip has to be against the person before they start. See
  the section on where-chip-codes-come-from.
- **Clear the "Not ready" list.** It is the two ways somebody silently gets no result — no chip and
  no number. A number can be given later; a chip really cannot.
- **Print the registration list twice** — by surname for the desk, by number for the line.

## The night before: charge everything

**Everything with a battery, including the ones nobody thinks of.**

- The boxes.
- The phone or laptop that will run all day. A phone holding radio links to boxes for six hours is
  working hard.
- Active tags are battery powered. If yours have been in a box since last season, test them, do not
  assume them.

Pack spares of the things that end a day when they fail: the dongle, a cable, a handful of tags,
and paper.

## On site: prove the line before the field arrives

**Walk a chip across every line and watch the reads come up.** It is the only test that proves the
whole chain at once — tag, antenna, box, clock, link and app.

- Do it at the **edges** of the line, not up the middle. The middle always works.
- Do it with a tag of **each kind** you are using. A box set for one kind does not read the other.
- Do it **again** after anything moves — a barrier, a vehicle, a gantry.

See the section on where-to-put-the-mat.

**A reader stops itself after two and a half minutes.** So a mat you tested at nine o'clock is not
still reading at ten. Start it when the race is about to go, not when you set it up.

## The last ten minutes

Four things, and a race that has all four cannot silently score nothing:

1. **The gun is set** for the wave that is about to go.
2. **The finish is named**, and named *exactly* as the point on the box is named. No fuzzy matching.
3. **The "Not ready" list is clear.**
4. **The box clocks are right**, or their corrections are entered. Read the box's clock against a
   clock you trust — see the section on correct-a-box-clock.

## And know your link's limits

Three connections at once over the network, one over Bluetooth. If somebody else is already on a
box, you are not getting on it. Decide before the day which device is timing.
...
- **Cannot stop** — a finish where people stand and get their breath back keeps reading them. That
  is handled (the section on one-time-looks-wrong), but it is handled by throwing
...
- **Water and bodies.** A wet shirt over a tag, a tag pinned against the back, a competitor
  ...
  the section on where-the-chip-goes.
...
- **Run cables where nobody treads.** Tape them down, or bring them in from behind the crowd line.
  ...
  reading (the section on box-connected-but-no-reads).
...
one gets a backup beside it (the section on back-everything-up).


---

# Where the chip goes on a competitor

**A chip that cannot be seen by the antenna cannot be read, however good the mat is.** More lost
times come from where the tag was worn than from anything wrong with the equipment.

## The rule behind all of it

**The tag wants to be on the outside of the person, clear of water and metal, and facing the way it
will pass the antenna.**

A UHF tag has no battery. The reader powers it and then listens for a very quiet answer, so a body,
a soaked shirt or a bike frame in the way costs you both halves of that exchange. An active tag
carries its own battery and announces itself, so it forgives far more — but a tag buried under three
layers and a race cape is asking for trouble either way.

## Common placements, and what each one costs

- **Ankle** — reliable, because it is low, clear of the body and close to a mat on the ground. It
  wants to be on the outside of the ankle, snug enough not to spin round and not so tight it marks.
- **On the bib** — convenient, because the number and the chip arrive together and nobody has to
  hand anything back. It depends on the bib staying on the **front** and staying flat. A bib folded
  up under a jacket, or turned to the back, is a tag that has gone quiet.
- **On the bike** — fork, frame or number plate, depending on the tag. Keep it off the frame's metal
  where you can and make sure it is not shrouded by a bottle, a bag or the rider's leg.
- **Swim** — wrist or ankle under the wetsuit sleeve is normal, but water is the hardest thing UHF
  has to fight. Talk to RFID Timing about the right tag before running a swim on tags you have not
  used in water.

## Tell the competitors, in one sentence

Most lost times are self-inflicted at registration. A line on the entry confirmation and a sign at
the desk does more good than anything you can do at the mat:

> Wear the tag on the outside, on the front, and do not fold, cut or cover it.

**Do not let anybody run with the tag in a pocket, under a bag strap, or pinned to the back**, and
do not let them cut a bib down to fit — the tag is usually part of what gets cut off.

## Handing them out

**A chip has to be against a person in Podium before they start**, not just in their hand. The
handing out and the recording are two jobs and it is the second one that produces results. See
the section on where-chip-codes-come-from.

- **Hand out in number order** where the tags are encoded in number order. Then the rule does the
  recording for you and there is nothing to type.
- **Every swap gets recorded at the moment it happens.** A tag swapped at the line and written on a
  hand is a tag that scores the wrong person. Do it there and then, one at a time on the entrant.
- **Check the "Not ready" list before the gun.** It is the list of people who will get no result,
  and no chip is the main way onto it.

## Getting them back

**Decide before the event whether you are collecting tags, and tell people at the finish.** A
collection funnel has to be part of the finish layout, not an afterthought once the winner is
through — see the section on running-a-finish.

Tags that walk home are next season's missing tags. If your club reissues the same tag to the same
racer each time, the carry-over route depends on getting them back
(the section on entrants-and-numbers).
...
**Everything is measured from the gun, so the gun is the one thing that has to be right.** A mat
that misses somebody costs one result. A gun that is wrong costs every result in the wave.

How to set it, and how to fix a wrong one, is the section on wave-starts. This page is about
the start itself.

## A start is the last read, and that is deliberate

**People roll onto the start mat and stand there waiting.** The first read is them arriving,
sometimes minutes early. The last read is them leaving. So Podium takes the **last** read of the
visit as the start.

Two things follow from that, and both bite people:

- **Reads taken before the gun are thrown away.** A finish mat parked near the start line will read
  the whole field standing around, and none of it counts.
- **Do not lengthen the gate on a start point.** A long gate merges a genuine second visit into the
  first one, so somebody who steps off and comes back gets the wrong start. See
  the section on timing-points.

## Keep the field off the mat until it is their wave

**With waves going off one after another, a competitor standing on the start mat during somebody
else's gun is a problem waiting to happen.** Hold each wave behind the line and let them on when it
is theirs.

This matters more than it sounds at a small event where everybody is milling about near the line.

## Do you even need a start mat?

**No.** A wave start needs a gun, not a mat — everybody in the wave is timed from that moment
whether or not anything read them.

A start mat buys you **chip time**: each person's time from when *they* crossed rather than from
the gun. That is worth having when the field is big enough that the back of it takes a while to
reach the line, and it is worth nothing at all in a small wave that starts together.

If you are not giving chip times, the start mat is one less thing to fail. Put the box on the finish
instead.

## Late starters

Somebody who misses the gun and sets off two minutes later gets a gun time measured from the gun,
because that is what a gun time is. If they should be timed from when they actually went, they need
a start read, which means a start mat — or a typed time to fill the gap
(the section on add-a-time-by-hand).

**Decide the club's rule on this before the day**, not while somebody is standing in front of you
asking about it.

## Before the gun, the three-item check

1. **Is the gun set for this wave?** Nothing scores without one, and Podium says so plainly
   (the section on nothing-scores-at-all).
2. **Is the finish named?** Same again — it will not guess which mat is the finish.
3. **Is the "Not ready" list clear?** That is the list of people who will get no result: no chip, or
   no number (the section on entrants-and-numbers).

All three are two minutes of work before the race and an unfixable afternoon afterwards.
...
**The finish is the one line you cannot redo.** A split can be reconstructed from a finish and a
pace. A missing finish is somebody's whole race.

## A finish is the first read, and that is deliberate

A mat reads a chip several times as somebody runs across it, and then keeps reading them while they
stand behind the line getting their breath back. **The moment they crossed is the first of those
reads**, so that is the one Podium takes.

The gate is what stops the standing-about reads becoming a second finish. See
the section on timing-points.

## Give them somewhere to go

**The single most useful thing at a finish is a funnel that keeps moving.** It is not about tidiness:

- People who stop on the mat sit in the read zone, which is work the system has to undo.
- People who wander back across the line can collect a read that looks like a second crossing.
- A bunch that bunches gets screened — bodies between tag and antenna is the main way a read is
  lost (the section on where-to-put-the-mat).

Walk them through, out the far side, and only then collect tags, hand out water, or take names.

## Bunch and sprint finishes

**Two people crossing together is not a problem for the mat — it is a problem for the read.** The
timing is fine to the millisecond; what fails is one tag being screened by the other body.

Three things help, in order of how much they help:

1. **A wide enough line** that shoulder-to-shoulder arrivals are not stacked front to back.
2. **Tags worn on the outside and on the front**
   (the section on where-the-chip-goes).
3. **Somebody writing the order down**, which is the only thing that saves you when the first two
   were not enough (the section on back-everything-up).

**Do not settle a close finish from the chip times alone if a placing is in dispute.** Go back to
the reads themselves — the section on raw-data.

## Watch the results as they come, not at the end

**A mat that has stopped reading looks exactly like a course with nobody on it.** The difference is
that you can still fix the first one while the race is running.

So keep the results screen in front of somebody, and treat a gap as a fault until proven otherwise:

- **Nobody at all for a while** — check the mat is still delivering
  (the section on box-connected-but-no-reads).
- **Several people from the same wave missing** — the mat, not the runners
  (the section on runner-shows-no-time).
- **One person missing** — usually the tag, and usually how it was worn.

## Collecting tags

If you are taking tags back, **the collection point goes after the funnel, not on the line.** People
stopping to unfasten an ankle strap in the read zone is the thing you were trying to avoid.

## Before you take the line down

1. **Is everybody accounted for?** Anybody still out on course, and anybody marked as not finished
   who actually did.
2. **Has the backlog reached zero?** That is the count of reads still to reach the server. Packing
   up mid-count is how a race arrives incomplete.
3. **Only then** unplug anything.
...
the section on raw-data. One number's reads will show it in seconds.
...
- **Battery and weather stop being background concerns.** See
  the section on weather-and-batteries.
- **Watch the results through the event, not at the end.** A mat that dies at hour three costs you
  ...
  the section on back-everything-up.
...
**The two things that end a race day are a flat battery and water in the wrong place.** Neither
announces itself, and both are entirely preventable the night before.

## Water is the enemy of a read

A UHF tag has no battery. The reader has to power it and then hear a very quiet answer back, and
**water absorbs exactly the signal doing both jobs.**

So rain does not break the equipment. It quietly costs you reads:

- **A soaked shirt over a bib tag** is a tag with water between it and the antenna.
- **Standing water on the line**, or saturated ground, changes what the antenna is working against.
- **A wet body** screens a tag better than a dry one, so a bunch finish in the rain is harder than
  the same finish in the dry.

**Test in the conditions you will run in.** A dry-morning test proves the dry morning. If the
forecast is wet, the useful test is a wet one — and if you cannot get one, plan the backup instead
(the section on back-everything-up).

An active tag carries its own battery and announces itself, so it forgives weather far better. If
you have a choice of tag for a wet event, that is the consideration.

## Keeping the kit alive

- **The box goes out of the weather.** It is the thing that must survive the day. Under cover, off
  the ground, out of standing water.
- **Cables are where water gets in**, and a connector sitting in a puddle is the one that fails at
  hour four. Bring them in from above where you can, and tape them down where people walk
  (the section on where-to-put-the-mat).
- **Condensation is real.** Kit brought from a cold vehicle into a warm marquee, or the reverse,
  gets wet on the inside. Give it time.

## Batteries

**Cold flattens a battery faster than the same day would warm.** A winter cross-country or a long
night section takes more out of everything than the number on the spec sheet suggests.

Charge everything the night before, including the ones nobody thinks of:

- **The boxes.**
- **The phone or laptop doing the timing.** Holding radio links to boxes all day is hard work for a
  phone, and it is the device you cannot do without.
- **The active tags.** They are battery powered. Tags that have been in a crate since last season
  get tested, not assumed.

**Watch the battery column rather than remembering to think about it.** Both apps show each box's
battery, and a box that is going flat tells you so long before it goes quiet. A box that has already
gone quiet looks identical to a box out of range or switched off
(the section on box-will-not-connect).

## Long events

For a twelve or twenty-four hour race, treat power as part of the plan rather than a check:

1. **Know the runtime**, and know it for the conditions — a cold night is not a mild afternoon.
2. **Have the changeover worked out before you need it**, including who does it and what happens to
   the reads while it happens. The box keeps its own log, so a box that is off the link is not
   losing anything (the section on recover-missed-reads).
3. **Check in through the event**, not at the end. See the section on lap-racing.

## If a box goes quiet anyway

**The reads are not lost.** Every box keeps its own log, so a box that timed a whole race flat on
its link still has everything on it. Deal with the link, then pull the reads off
(the section on reads-stopped-during-a-race).

What you cannot recover is a read the box never took. That is why the backup matters and why the
tag placement matters — a flat box is fixable, a tag nobody could read is not.
...
**Every time should be recorded two ways.** Not because the equipment is unreliable, but because the
one thing you cannot do after the event is watch the race again.

A missed read is recoverable if something else saw the same person. It is gone if nothing did.

## The backups you already have, free

**Every box keeps its own log.** A box that times an entire race with nothing connected to it has
still recorded the lot, and it can be pulled off afterwards. A dropped link is an inconvenience, not
a loss — see the section on recover-missed-reads.

That means **a box failing to connect is never a reason to stop the race.** Let it read. Sort the
link out afterwards.

## A second box on the same line

Two boxes can cover one point. Add the second one and say which it is:

- **Its reads are crossings** — a genuine second mat, for a wide line or a busy one. Both count.
- **Watching it, never a crossing** — it records but never gives anybody a time.

Put a backup on the finish before you put one anywhere else. See
the section on timing-points.

## A spotter box

**A spotter never gives anybody a time**, which is exactly what makes it safe to put anywhere. Its
job is to see people. Put one before another point in the running order and it will back that point
up, and it is what feeds a commentator without any risk of contributing a wrong result.

## The human backup

**Somebody writing down the finish order, on paper, is worth more than it sounds.** It is the thing
that rescues a race when a mat fails outright, and it costs one volunteer and a clipboard.

Two rules make it useful rather than decorative:

1. **Record the order, and record times if you can.** Order alone rebuilds a result. Order plus a
   handful of times rebuilds it accurately.
2. **A typed time only ever fills a gap.** It never overrules a chip read, deliberately — the box
   stamped the chip to the millisecond, and a person wrote down when they thought they saw
   somebody. See the section on add-a-time-by-hand.

A phone video of the finish, running continuously, is the same idea with less work. Start it before
the leader arrives and leave it running.

## Before you pack up

**Wait for the backlog to reach zero.** That is the count of reads still waiting to reach the
server, and packing up while it is counting down is how a race arrives at the office incomplete.

Then, and only then, take the line apart.

## What this actually protects you from

The three that happen:

- **A mat that quietly stopped reading.** Nothing alarms, because a reader with a dead antenna
  reports no fault (the section on box-connected-but-no-reads).
- **A person whose tag was never going to be read** — worn wrong, covered, or never issued
  (the section on where-the-chip-goes).
- **A bunch finish** where several people crossed in the same second and one of them was screened by
  the others.

None of the three is fixable at the time. All three are survivable if something else was watching.
...
Where to find them is the section on raw-data. This page is how to use them.
...
- A clock correction **belongs to the box, not the timing point.** Two boxes can cover one line and
  only one of them may be wrong. See the section on correct-a-box-clock.
...
them (the section on back-everything-up).
